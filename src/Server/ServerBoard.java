 ...
 * ServerBoard class
 * 
 * A classe ServerBoard e utilizada para construir e atualizar o tabuleiro no
 * servidor, consoante as informacoes enviadas pelo cliente.
 * Cada ServerBoard tem um numero de linhas, colunas e navios
 * 
 * 
 */
public class ServerBoard {
	...
	/**
	 * ServerBoard construtor
	 * 
	 * @param row      O numero de linhas do ServerBoard
	 * @param col      O numero de colunas do ServerBoard
	 * @param numShips O numero de navios do ServerBoard
	 * @return uma instancia da classe ServerBoard
	 */
	public ServerBoard(int row, int col, int numShips) {
		this.col = col;
		this.row = row;
	 ...
	 * Obter o tabuleiro
	 * 
	 * @return O tabuleiro do ServerBoard
	 */
	 ...
	 * Muda o tabuleiro
	 * 
	 * @return O novo tabuleiro do ServerBoard
	 */
	...
	/**
	 * Obter o numero de colunas do ServerBoard
	 * 
	 * @return O numero de colunas do ServerBoard
	 */
	public int getCol() {
		return col;
	}
	...
	/**
	 * Muda o numero de colunas do ServerBoard
	 * 
	 * @return O novo numero de colunas do ServerBoard
	 */
	...
	/**
	 * Obter o numero de linhas do ServerBoard
	 * 
	 * @return O numero de linhas do ServerBoard
	 */
	public int getRow() {
		return row;
	}
	...
	/**
	 * Muda o numero de linhas do ServerBoard
	 * 
	 * @return O novo numero de linhas do ServerBoard
	 */
	...
	/**
	 * Obter o numero de navios do ServerBoard
	 * 
	 * @return O numero de navios do ServerBoard
	 */
	...
	/**
	 * Muda o numero de navios do ServerBoard
	 * 
	 * @return O novo numero de navios do ServerBoard
	 */
	...
	public void createServerBoard() {
		// Enche o tabuleiro com agua e com o numero de navios pre-definido

		int ship_type; // Criacao do objeto inteiro ship_type
		int orientation; // criacao do objeto inteiro orientation
		int[] spot = new int[2]; // criacao de um vetor de numeros inteiro do tipo 1*2
		int attempt = 0; // criacao do objeto inteiro attempt
		int placedShips = 0; // criacao do objeto inteiro placedShips

		char possibleSpot; // criacao dos objetos tipo possibleSpot
		char possibleSpot1;
		char possibleSpot2;
		char possibleSpot3;
		char possibleSpot4;
		...
		while (placedShips < this.getNumShips() && attempt <= 100) // enquanto a contagem de navios for inferior ao
																	// numero de
																	// navios colocados pelo utilizador e inferior ou
																	// igual
																	// a 100 tentativas
		{
			...
			{
				orientation = 0; // A orientacao nao tera influencia uma vez que este tipo de barco e constituido
									// apenas
									// por um elemento
				spot = generateCoordinates(ship_type, orientation, this.getRow(), this.getCol()); // atraves da funcao
																									// gerarCoordenadas2,
																									// obtemos
				// um vetor [c1 c2] que nos indicara a
				// localizacao do nosso possivel lugar
				possibleSpot = this.board[spot[0]][spot[1]]; // o possivel local e colocado sobre a matriz this.board

				if (possibleSpot == water) // caso este possivel local na matriz esteja ocupado por agua
				{
					this.board[spot[0]][spot[1]] = ship; // passa a ser substituido na matriz por um objeto navio
					placedShips++; // aumenta em 1 a contagem dos navios colocados
				}
			}
			...
			else if (ship_type == 1) // Caso o navio seja do tipo 1 (SS)
			{
				...
				spot = generateCoordinates(ship_type, orientation, this.getRow(), this.getCol()); // Selecionado o tipo
																									// de navio e
																									// orientacao foi
				// gerada a primeira das suas coordenadas,
				// recorrendo a funcao generateCoordinates
				// iremos obter rum vetor [c1 c2] que nos
				// dara a posicao do possivel local para a
				// uma celula do navio
				...
				else // se nenhuma das outras posicoes for escolhida entao passamos para a posicao 4
						// (oeste)
				{
			...
			else if (ship_type == 2) // se o navio escolhido for o navio tipo 2 (SSSS)
			{
				...
				spot = generateCoordinates(ship_type, orientation, this.getRow(), this.getCol());// atraves do
																									// gerarCoordenadas
																									// geramos a posicao
				// da primeira celula
	...
	public static int[] generateCoordinates(int ship_type, int orientation, int row, int col) {
		// Atraves desta funcao o utilizador ira gerar um vetor de duas coordenadas onde
		// serao colocados os navios de
		...
		int c1 = 0; // c1 vai ser o primeiro elemento do vetor ship_coordinates, correspondente à
					// linha
		int c2 = 0; // c2 vai ser o segundo elemento do vetor ship_coordinates, correspondente à
					// coluna

		if (ship_type == 0) // Caso o navio seja do tipo 0 ( ou seja apenas ocupa uma celula da matriz
							// tabuleiro)
		{
			...
			// Uma vez que o navio tipo 0 ocupa somente uma celula nao e necessario definir
			// a sua orientacao
		...
		else if (ship_type == 1) // No caso do navio tipo 1 ( que ocupa duas celulas seguidas na tabela)
		{
			...
			// Neste caso, e uma vez que o navio e composto por mais do que uma celula temos
			// que avaliar a sua direcao
			// (norte(0),sul(1),este(3) e oeste(4))

			if (orientation == 0) // Caso a sua orientacao seja para norte / para cima)
			{
				while (c1 < 1) // Se c1 'calhar' na primeira linha, o navio ficara fora do limite superio da
								// matriz
				{
			...
			else if (orientation == 1) // Se a orientacao for para sul( para baixo)
			{
				while (c1 >= (row - 1)) // se c1 for colocado na ultima linha de matriz, o navio estara fora do limite
										// inferior da matriz
				{
					c1 = new Random().nextInt(row); // continuaremos a gerar valores aleatorios para c1 ate que este
													// seja
		...
		else // No caso do tipo de barco ser o tipo 2, ou seja, ocupando 4 celulas seguidas
				// da matriz
		{
			...
			{
				while (c1 < 3) // e se c1 'calhar' em alguma das primeiras 3 linhas, o navio encontrar-se-a
								// fora do
...

	{
		...
		// coordenadas iguais as inseridas pelo utilizador
		String result = "";
		String finalResult = "";
		...
		finalResult = "SHOT_RESULT=" + result + ";" + message;
	...
	public String findShipLocations() {
		// Cria uma string com as coordenadas das celulas correspondentes a navios
		...
		// Retira o ultimo ";" para evitar erros de leitura
